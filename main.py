...
runs = st.sidebar.slider("Runs per Scenario", 1, 200, 30)#The number of times each scenario is run before averaging the simulation results
tariff = st.sidebar.slider("Tariff ($/kWh)", 0.05, 0.50, 0.20)#The price per kilowatt-hour (kWh) that households pay for electricity, where higher values may indicate a greater financial burden on households and lower values may indicate a more affordable energy access.
...
no_of_households = st.sidebar.number_input("Households", 1, value=100)#Total number of households within the community that the microgrid can serve
stoptime = st.sidebar.slider("Simulation Time (months)", 20, 180, 120)#The total duration of the simulation in months, where a longer time horizon allows for observing long-term impacts but may also introduce more uncertainty.
...
microgrid_capacity = st.sidebar.number_input("Microgrid Capacity (kW)", 1, value=1000000)#The maximum capacity of the microgrid in kilowatts (kW)
...
def run_scenario(runs):#Runs the impact estimator model for a given scenario and number of runs, returning a list of results for each run that can be averaged later to account for variability in the simulation outcomes.
    ...
    for _ in range(runs):
        ...
        series = {
            ...
            "co2": [results[t].get("co2_emissions",0) for t in times],#The amount of CO2 emissions in kilograms at each time point, which can reflect the environmental impact of the microgrid, where lower values indicate a more environmentally friendly energy solution.
...
metric = st.selectbox(
    "Metric",
    ["connected","income($)","income_women($)","income_men($)","schooling(years)","co2","reliability","saidi(hours)","impact","welfare","dswf"]
)
